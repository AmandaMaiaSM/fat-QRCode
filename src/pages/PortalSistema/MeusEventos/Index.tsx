...
export default function MeusEventos() {
  ...
  const [adicionarParticipanteAtivo, setAdicionarParticipanteAtivo] =
    useState(false);
  ...
  const [idParticipanteEditando, setIdParticipanteEditando] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    const eventosSalvos = JSON.parse(
      localStorage.getItem("meus_eventos") || "[]",
    );
  ...
  const adicionarParticipante = (p0: boolean) => {
    if (
      ...
      !novoParticipante.email ||
      !novoParticipante.hora
    )
      return;
  ...
  const handleAbrirAdicionarParticipante = (pessoa: {
    ...
    setNovoParticipante(
      pessoa
        ? { nome: pessoa.nome, email: pessoa.email, hora: pessoa.hora }
        : { nome: "", email: "", hora: "" },
    );
  ...
  const handleAbrirConfirmacao = (id: React.SetStateAction<string | null>) => {
    setIdParaExcluir(id);
  ...
  const salvarEdicao = () => {
    setEventos((prev) =>
      prev.map((ev) =>
        ev.id === dadosEdicao.id
          ? { ...ev, nome: dadosEdicao.nome, data: dadosEdicao.data }
          : ev,
      ),
    );
  ...
  const handleExcluirParticipante = (idParticipante: string) => {
    ...
    setEventoSelecionado((evnt) => {
      ...
      return {
        ...evnt,
        participantes: evnt.participantes.filter(
          (p) => p.id !== idParticipante,
        ),
      };
    });
          ...
          return {
            ...
            participantes: ev.participantes.map((p) =>
              p.id === idParticipanteEditando
                ? {
                    ...p,
                    nome: novoNomeParticipante,
                    email: novoEmailParticipante,
                    hora: novaHoraParticipante,
                  }
                : p,
            ),
          };
  ...
  return (
    <div className="dashboard-container meus-eventos-page">
      <main className="main-content meus-eventos-main">
        ...
        {/*Modal de lista de presença  nao tem uma pagina propria  */}
              ...
              <div className="modal-body">
                <>
                  {adicionarParticipanteAtivo && (
                    <div
                      className={`add-participant-form ${adicionarParticipanteAtivo ? "active" : ""}`}
                    >
                      ...
                      <input
                        ...
                        className="input-Add-participante"
                      />
                      <label htmlFor="hora">Hora de check-in :</label>
                      <input
                        type="time"
                      ...
                      <div className="add-participante-actions">
                        <button
                          className="btn-save"
                          onClick={() => adicionarParticipante(false)}
                        >
                          Adicionar
                        </button>
                        <button
                          className="btn-cancel"
                          onClick={handleCancelarAdicionarParticipante}
                        >
                          Cancelar
                        </button>
                      </div>
                  ...
                  {eventoSelecionado.participantes.length > 0 ? (
                    <table className="attendess-table">
                      <thead>
                        <tr>
                          ...
                          {(eventoSelecionado.camposPersonalizados ?? []).map(
                            (campo) => (
                      ...
                      <tbody className="participantes-table-body">
                        {eventoSelecionado.participantes.map((pessoa) => (
                          ...
                          <tr key={pessoa.id}>
                            <td data-label="Nome">
                              {idParticipanteEditando === pessoa.id ? (
                                <input
                                  ...
                                  onChange={(e) =>
                                    setNovoNomeParticipante(e.target.value)
                                  }
                            ...
                            <td data-label="Email">
                              {idParticipanteEditando === pessoa.id ? (
                                <input
                                  ...
                                  onChange={(e) =>
                                    setNovoEmailParticipante(e.target.value)
                                  }
                                />
                              ) : (
                                pessoa.email
                            ...
                            <td data-label="Hora">
                              {idParticipanteEditando === pessoa.id ? (
                                <input
                                  ...
                                  onChange={(e) =>
                                    setNovaHoraParticipante(e.target.value)
                                  }
                                />
                              ) : (
                                pessoa.hora
                            ...
                            {(eventoSelecionado.camposPersonalizados ?? []).map(
                              (campo) => (
                                <td key={campo.id} data-label={campo.label}>
                                  {String(pessoa[campo.name] ?? "-")}
                                </td>
                              ),
                            )}
                            ...
                            <td>
                              {idParticipanteEditando === pessoa.id ? (
                                <>
                                  <button
                                    className="btn-save"
                                    onClick={salvarNomeParticipante}
                                  >
                                    Salvar
                                  </button>
                                  <button
                                    className="btn-cancel"
                                    onClick={handleCancelarEdicaoParticipante}
                                  >
                                    Cancelar
                                  </button>
                                </>
                                ...
                                <>
                                  ...
                                  <button
                                    ...
                                    title="Adicionar"
                                    onClick={() =>
                                      handleAbrirAdicionarParticipante(pessoa)
                                    }
                                  >
                                    <CirclePlus />
                                  </button>
                                  <button
                                    ...
                                    title="Excluir"
                                    onClick={() =>
                                      handleExcluirParticipante(pessoa.id)
                                    }
                                  >
                                    <Trash2 size={16} />
                                  </button>
                    ...
                    <p className="empty-state">
                      Ninguém realizou check-in ainda.
                    </p>
                  )}
                </>
              </div>
